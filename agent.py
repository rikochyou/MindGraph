...
class Agent():
    ...
    def graph_func(self, x, query):
        ...
        for document in graph_documents_filter:
            ...
            try:
                records, summary, keys = driver.execute_query(cypher, database_="doctor")
                #for record in records:
                    #print(record.data())  # obtain record as dict
                # if result and any(value for value in result[0].values()):
                #     answer_str = replace_token_in_string(answer, list(result[0].items()))
                #     query_result.append(f'问题：{question}\n答案：{answer_str}')
                if records and any(value for value in records):
                    res = []
                    for record in records:
                        res.append(record.data())
                    answer_str = replace_graph_token(answer, res[0])
                    query_result.append(f'问题：{question}\n答案：{answer_str}')
...
if __name__ == '__main__':
    ...
    # print(agent.query('焦虑症吃什么药？'))

    # print(agent.generic_func('', '你好'))
    # print(agent.generic_func('你叫什么名字？'))
    # print(agent.generic_func('', 'CBT如何帮助家长解决亲子教育中的问题？'))
    # print(agent.generic_func('', '焦虑症吃什么药？'))
    
    # print(agent.retrival_func('介绍一下寻医问药网'))
    # print(agent.retrival_func('寻医问药网的客服电话是多少？'))
    # print(agent.retrival_func('','感到孤独怎么办？'))
    # print(agent.retrival_func('','如何保持社交联系？'))
    print(agent.retrival_func('','焦虑症吃什么药？'))
    
    # print(agent.graph_func('感冒和鼻炎是并发症吗？'))
    # print(agent.graph_func('','抑郁的症状是什么？'))
    # print(agent.graph_func('感冒吃什么药好得快？可以吃阿莫西林吗？'))
    # print(agent.graph_func('','焦虑症吃什么药？'))

    # print(agent.search_func('抑郁症是什么？'))